...
const permissionOptions = [
  ...
  { key: "teachers", label: "Teachers", description: "Assign and manage teaching staff", icon: Users },
  { key: "feedback", label: "Feedback", description: "Review and manage customer feedback", icon: MessageSquare },
  { key: "promotions", label: "Promotions", description: "Manage marketing and promotional campaigns", icon: Megaphone }
];
  ...
  useEffect(() => {
    if (member) {
      setPermissions({
        ...
        teachers: member.permissions?.teachers || false,
        feedback: member.permissions?.feedback || false,
        promotions: member.permissions?.promotions || false
      });
    }
  }, [member]);
...
const TeamWidget = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const response = await fetch("https://crm-4xul.onrender.com/api/members/getMembers/Marketing");
  ...
  const handleToggleApprove = async (id) => {
    try {
      ...
      const response = await fetch(`https://crm-4xul.onrender.com/api/members/updateApproval/Marketing/${id}`, {
        method: "PATCH",
  ...
  const handleSavePermissions = async (memberId, permissions, team) => {
    try {
      const response = await fetch(`https://crm-4xul.onrender.com/api/members/updatePermissions/${memberId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },

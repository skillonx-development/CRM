...
const TeamWidget = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const response = await fetch("/api/members/getMembers/Marketing");
  ...
  const handleToggleApprove = async (id) => {
    try {
      ...
      const response = await fetch(`/api/members/updateApproval/Marketing/${id}`, {
        method: "PATCH",
  ...
  const handleSavePermissions = async (memberId, permissions, team) => {
    try {
      const response = await fetch(`/api/members/updatePermissions/${memberId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },

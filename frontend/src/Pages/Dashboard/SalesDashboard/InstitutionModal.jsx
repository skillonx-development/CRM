...
const ManageData = () => {
  ...
  const [states, setStates] = useState([]); // Changed from static import to state
  ...
  const fetchStates = async () => {
    try {
      const response = await fetch("http://localhost:5001/api/cowin/states");
  ...
  useEffect(() => {
    const fetchDistricts = async () => {
      if (selectedState) {
        try {
          console.log('Fetching districts for state ID:', selectedState);
          
          const response = await fetch(`http://localhost:5001/api/cowin/districts/${selectedState}`);
          const data = await response.json();
          
          if (response.ok && data.districts) {
            // Normalize district IDs to strings for consistency
            const normalizedDistricts = data.districts.map((d) => ({
              ...d,
              district_id: String(d.district_id),
            }));
            setDistricts(normalizedDistricts);
            console.log('Districts loaded:', normalizedDistricts.length);
          } else {
            console.warn("No districts found in response:", data);
            setDistricts([]);
            showToast("No districts found for selected state", "error");
          }
        } catch (error) {
        ...
        setDistricts([]);
        setSelectedDistrict(""); // Reset district selection when no state is selected
      }
    };

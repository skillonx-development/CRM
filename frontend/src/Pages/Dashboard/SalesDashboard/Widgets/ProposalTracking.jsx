...
const api = {
  fetchProposals: async () => {
    try {
      const response = await axios.get('https://crm-r5rr.onrender.com/api/proposals');
  ...
  createProposal: async (proposal) => {
    try {
      const response = await axios.post('https://crm-r5rr.onrender.com/api/proposals', proposal);
  ...
  updateProposal: async (id, proposal) => {
    try {
      const response = await axios.put(`https://crm-r5rr.onrender.com/api/proposals/${id}`, proposal);
      return response.data.proposal;
    } catch (error) {
...
const ProposalTracking = () => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`https://crm-r5rr.onrender.com/api/proposals/${editProposal._id}`, {
        method: "PUT",
        headers: {
